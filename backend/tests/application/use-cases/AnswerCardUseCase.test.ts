...
describe('AnswerCardUseCase', () => {
    ...
    const useCase = new AnswerCardUseCase(mockCardRepo, mockReviewRepo, mockQuizzUseCase);
    ...
    it('should process correct answer successfully', async () => {
        ...
        jest.spyOn(LeitnerService, 'updateCardCategory').mockResolvedValue(updatedCard);
    ...
    it('should handle forced validation with incorrect answer', async () => {
        // Arrange
        const card = new Card('1', Category.FIRST, 'Q', 'A');
        const dto = new AnswerCardDTO(false, true);
        ...
        expect(mockReviewRepo.save).toHaveBeenCalledWith(
            expect.objectContaining({
                cardId: '1',
                isCorrect: true, // Devrait être true car forcé
                forcedValid: true
            })
        );
    });

    it('should fail for incorrect answer without forced validation', async () => {
        // Arrange
        const card = new Card('1', Category.FIRST, 'Q', 'A');
    ...
    it('should prevent multiple reviews of same card in one day', async () => {
        ...
        await expect(useCase.execute('1', dto))
            .rejects
            .toThrow('Card already reviewed today');
    });
    ...
    it('should throw error if card is not in today\'s quiz', async () => {
        ...
        await expect(useCase.execute('1', dto))
            .rejects
            .toThrow('Card is not part of today\'s quiz');
    });
