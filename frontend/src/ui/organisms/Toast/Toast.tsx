...
export function Toast() {
  ...
  React.useEffect(() => {
    ...
    if (currentToast.sideOffset) {
      toast.style.setProperty("--toast-sideOffset", currentToast.sideOffset.toString() + "px");
    }
        ...
        <section>
          <ol>
            {toast.map((t: ToastProps, index: number) => {
              return (
                <li
                  className={`toast flex flex-row items-center border-solid border-[1px] border-gray-200 justify-center fixed p-3 shadow-sm rounded-xl min-w-[18.25rem] max-w-[26rem] ${style[t.style].container}`}
                  ref={currentToastRef}
                  data-id={index}
                  data-x-position={toastPosition?.x}
                  data-y-position={toastPosition?.y}
                  key={index}>
                  <div className={"w-full flex flex-row gap-x-4 items-center pr-3"}>
                    {t.style === "info" && <span>{ToastIcons.ToastIconInfo}</span>}
                    {t.style === "success" && <span>{ToastIcons.ToastIconSucess}</span>}
                    {t.style === "error" && <span>{ToastIcons.ToastIconError}</span>}
                    {t.style === "warning" && <span>{ToastIcons.ToastIconWarning}</span>}
                    <div className={"flex flex-col"}>
                      <h4 className={`text-sm font-semibold text-gray-800 w-full ${style[t.style].title}`}>
                        {t.title}
                      </h4>
                      {t.description && 
